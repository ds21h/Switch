...
const static char *TAG = "xesp_ota_ops";
...
{
    ESP_LOGI(TAG, "Ota Begin enhanced version");
    ...
    ESP_LOGI(TAG, "Verify partition");
    partition = esp_partition_verify(partition);
    if (partition == NULL) {
        ESP_LOGI(TAG, "Invalid partition");
        return ESP_ERR_NOT_FOUND;
    }

    ESP_LOGI(TAG, "Test OTA partition");
    if (!is_ota_partition(partition)) {
        ESP_LOGI(TAG, "Invalid partition");
        return ESP_ERR_INVALID_ARG;
    }

    ESP_LOGI(TAG, "Test other partion than running");
    if (partition == esp_ota_get_running_partition()) {
        ESP_LOGI(TAG, "Partition conflict");
...
/*    if ((image_size == 0) || (image_size == OTA_SIZE_UNKNOWN)) {
    ...
    if ((image_size == 0) || (image_size == OTA_SIZE_UNKNOWN)) {
        ESP_LOGI(TAG, "Erase complete partition");
    	lSize = partition->size;
    } else {
        ESP_LOGI(TAG, "Erase partial partition");
    	lSize = (image_size / SPI_FLASH_SEC_SIZE + 1) * SPI_FLASH_SEC_SIZE;
    }
    ESP_LOGI(TAG, "%d bytes to erase", lSize);
    lStart = 0;
	lErase = SPI_FLASH_SEC_SIZE;
    while (lSize > 0){
    	if (lSize < lErase){
    		lErase = lSize;
    	}
		vTaskDelay(2);
	    ESP_LOGI(TAG, "Erasing %d bytes", lErase);
        ret = esp_partition_erase_range(partition, lStart, lErase);
    	if (ret == ESP_OK){
    		lSize -= lErase;
    		lStart += lErase;
    	    ESP_LOGI(TAG, "%d bytes erased, %d to go", lErase, lSize);
    	} else {
    	    ESP_LOGI(TAG, "Erase error %d", ret);
    	    break;
    	}
    }
